        ...
        const { data } = await axios
            .get(url, {
                headers: {
                    ...
                    "public-key": process.env.SANDBOX_VT_PUBLIC_KEY,
                },
            })
            .then((res) => res)
...
const verify_smile_number = async (request, response) => {
    ...
    try {
        ...
        if (phone === "") {
            response
                .status(400)
                .json({ message: "Kindly fill out smile phone number" });
            return;
        }
        ...
        const { data } = await axios
            .post(
                url,
                { billersCode: phone, serviceID: "smile-direct" },
                {
                    headers: {
                        "api-key": process.env.SANDBOX_VT_API_KEY,
